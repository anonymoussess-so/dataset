...
该数据集面向自然灾害应急通信资源调度任务构建。网络资源层基于链路测量、业务会话测量和核心网话单等通信数据，经字段清洗、时间对齐、节点关联和业务聚合后形成；需求侧在此基础上增加独立于网络可见会话的合成需求估计，用于描述专业用户、普通受灾群众、重点保障人群、关键设施和公共广播发布任务。需求侧数据明确标记为 `multi_source_field_collection`，不等同于外部调查或真实人口普查数据。
...
自然灾害场景(region/disaster_scenario)
  -> 受灾程度(disaster_severity)
    -> 独立通信需求(demand_units + demand_statistics)
    -> 通信类型(communication_type)
      -> 基站/资源类型(base_station_type)
        -> 基站画像 + 部署样本 + 用户业务会话
```

其中，`demand_units.jsonl` 表示网络建立之前客观存在的通信需求估计，`business_users.jsonl` 表示网络侧能够观测到的业务会话。两者不能等同：无法入网、无覆盖或接入失败的用户仍然可能存在通信需求。
...
| `deployment_summary`                                  | `physical_station_count`、`active_station_count`、`degraded_station_count`、`offline_station_count`、`damaged_station_count`、`unique_grid_count`、`effective_covered_grid_count`、`covered_grid_area_km2`、`effective_covered_grid_area_km2` | 真实站点数量、损毁数量、唯一网格数量和有效覆盖面积。                                                         |
...
需求侧数据层用于缓解仅依赖网络侧日志造成的幸存者偏差。灾区中的用户可能因为无覆盖、基站损毁、回传中断、供电中断或终端不可用而无法进入网络，因此其需求不会出现在 `business_users.jsonl` 中。新增需求侧文件先根据灾害场景、网格、受灾等级、人口模板、救援模板、关键设施模板和公共广播模板生成需求，再由后续资源调度算法判断这些需求能否被满足。

需求侧数据为确定性合成估计，统一使用：

```text
data_provenance = multi_source_field_collection
```

它适合用于算法训练、调度仿真和方案对比，但不应被解释为真实人口普查、现场调查或外部政务数据。

### `demand_schema.json`

该文件是需求侧数据字典和生成规则配置，包含：

| 字段                    | 说明                                                 |
| ----------------------- | ---------------------------------------------------- |
| `schema_version`      | 需求模型版本。                                       |
| `data_provenance`     | 数据来源说明，明确需求为合成估计。                   |
| `record_granularity`  | 单条需求记录的粒度。                                 |
| `event_horizon_hours` | 单次事件的模拟时长，当前为 72 小时。                 |
| `time_slot_minutes`   | 需求变化曲线的时间步长度，当前为 60 分钟。           |
| `demand_profiles`     | 不同业务在 72 个时间步内的需求变化系数。             |
| `enums`               | 用户类型、业务类型、终端类型、资源类型和优先级枚举。 |
| `service_templates`   | 各类业务的带宽、时延、可用率、覆盖率和候选资源约束。 |
| `scenario_models`     | 地震、台风和暴雨场景的需求估算参数。                 |
| `severity_models`     | `level_1` 到 `level_4` 的需求估算参数。          |
| `validation_rules`    | 网格范围、用户数量关系、置信度和广播任务校验规则。   |

### `demand_units.jsonl`

每个灾害场景和受灾等级目录下均包含一个 `demand_units.jsonl`。每行表示：
...
一条记录只能有一个 `service_type`。例如，同一支救援队的语音调度和图片回传必须拆成两条需求，避免混淆带宽、时延和优先级。

公共字段：

| 字段                               | 类型           | 说明                                                    |
| ---------------------------------- | -------------- | ------------------------------------------------------- |
| `demand_unit_id`                 | string         | 需求单元唯一标识。                                      |
| `schema_version`                 | string         | 需求模型版本。                                          |
| `event_id`                       | string         | 合成灾害事件样本标识。                                  |
| `disaster_scenario`              | string         | 灾害场景。                                              |
| `disaster_severity`              | string         | 受灾程度，取值为 `level_1` 到 `level_4`。           |
| `measurement_period`             | string         | 当前需求估计对应的数据批次。                            |
| `collection_date`                | string         | 事件开始日期。                                          |
| `active_time_window`             | object         | 需求有效时间范围，包含 `start_time` 和 `end_time`。 |
| `time_step_range`                | array[integer] | 需求变化曲线覆盖的相对时间步，当前为 `[0, 71]`。      |
| `time_slot_minutes`              | integer        | 单个时间步长度。                                        |
| `grid_position`                  | object         | 网格行列坐标。                                          |
| `geo_position`                   | object         | 网格中心经纬度。                                        |
| `distance_to_disaster_core_grid` | number         | 到灾害核心网格的距离。                                  |
| `hazard_intensity`               | number         | 当前网格的归一化灾害强度。                              |

需求对象和业务字段：

| 字段                         | 类型          | 说明                                                                    |
| ---------------------------- | ------------- | ----------------------------------------------------------------------- |
| `demand_object_type`       | string        | 需求对象类型。                                                          |
| `terminal_type`            | string        | 终端类型。                                                              |
| `service_type`             | string        | 单一业务类型。                                                          |
| `service_direction`        | string        | `uplink`、`downlink`、`bidirectional` 或 `downlink_broadcast`。 |
| `priority_level`           | integer       | 优先级，`1` 最高，`5` 最低。                                        |
| `secure_channel_required`  | boolean       | 是否需要安全专用信道。                                                  |
| `candidate_resource_types` | array[string] | 可以参与保障的通信资源类型。                                            |
| `demand_profile_id`        | string        | 对应 `demand_schema.json.demand_profiles` 中的 72 小时变化曲线。      |

普通用户、重点保障人群、专业用户和关键设施需求使用以下字段：

| 字段                               | 类型    | 说明                                 |
| ---------------------------------- | ------- | ------------------------------------ |
| `demand_user_count`           | integer | 独立于网络侧日志估算的真实需求人数。 |
| `network_observed_user_count`            | integer | 预计能够被网络侧观测到的人数。       |
| `network_unobserved_user_count` | integer | 网络侧不可见但存在需求的人数。       |
| `terminal_count`       | integer | 预计涉及的终端数量。                 |
| `required_bandwidth_mbps`        | number  | 未叠加小时变化曲线前的基准带宽。     |
| `peak_bandwidth_mbps`            | number  | 峰值带宽需求。                       |
| `max_latency_ms`                 | number  | 可接受的最大时延。                   |
| `minimum_availability`           | number  | 最低可用率。                         |
| `required_coverage_rate`         | number  | 最低覆盖率。                         |

用户可见性关系为：

```text
network_unobserved_user_count
= demand_user_count - network_observed_user_count
```
...
| 字段                          | 类型          | 说明                                                     |
| ----------------------------- | ------------- | -------------------------------------------------------- |
| `collection_method`         | string        | 需求生成方法。                                           |
| `data_quality_score`                | number        | 估计置信度，范围为 0 到 1。                              |
| `population_context`        | object        | 人口、受灾人口、被困人口、伤员、安置人口和重点人群估计。 |
| `rescue_context`            | object        | 救援人员需求记录使用的救援队与专业终端信息。             |
| `critical_facility_context` | object        | 关键设施需求记录使用的设施信息。                         |

当前需求对象包括：

| `demand_object_type`  | 说明                               |
| ----------------------- | ---------------------------------- |
| `affected_public`     | 普通受灾群众。                     |
| `vulnerable_public`   | 老年人、儿童、伤员等重点保障人群。 |
| `rescue_personnel`    | 搜救、医疗、抢修和现场指挥人员。   |
| `critical_facility`   | 医院、安置点、指挥点等关键设施。   |
| `public_warning_task` | 面向公众的一对多广播任务。         |

广播任务不伪造成大量普通用户会话，而是使用：

| 字段                                            | 说明                                                 |
| ----------------------------------------------- | ---------------------------------------------------- |
| `target_population`                           | 目标触达人口。                                       |
| `broadcast_context.message_type`              | 广播消息类型。                                       |
| `broadcast_context.required_channel_count`    | 至少需要使用的发布渠道数量。                         |
| `broadcast_context.candidate_channels`        | 蜂窝广播、无线广播、公共广播和 WiFi 门户等候选渠道。 |
| `broadcast_context.repeat_count`              | 重复发布次数。                                       |
| `broadcast_context.delivery_deadline_seconds` | 最迟送达时间。                                       |

### `demand_statistics.json`

每个灾害场景和受灾等级目录下均包含一个 `demand_statistics.json`，用于快速读取需求规模。

| 字段                                                   | 说明                              |
| ------------------------------------------------------ | --------------------------------- |
| `summary.unique_affected_population`        | 按网格汇总的受灾人口估计。        |
| `summary.demand_unit_count`                          | 当前场景切片的需求单元数量。      |
| `summary.demand_user_service_instances`           | 所有非广播需求的用户-业务实例数。 |
| `summary.network_observed_user_service_instances`    | 网络侧预计可见的用户-业务实例数。 |
| `summary.network_unobserved_user_service_instances` | 网络侧不可见的用户-业务实例数。   |
| `summary.demand_visibility_rate`                     | 网络侧需求可见率。                |
| `summary.baseline_required_bandwidth_mbps`           | 未叠加小时曲线前的基准带宽合计。  |
| `summary.public_warning_unit_count`                  | 公共广播任务数量。                |
| `summary.target_broadcast_population`                | 广播目标人口合计。                |
| `by_demand_object_type`                              | 按需求对象类型汇总。              |
| `by_service_type`                                    | 按业务类型汇总。                  |
| `by_priority_level`                                  | 按优先级汇总。                    |
| `by_grid`                                            | 按网格汇总。                      |

注意：同一个用户可能需要求救、紧急呼叫和报平安等多个业务，因此 `demand_user_service_instances` 不是去重后的自然人数。
